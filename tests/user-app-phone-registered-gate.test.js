...
test('USER APP: Registered Phone Database Gate & No Basic Details for Unregistered Numbers', async (t) => {
  ...
  await t.test('11. epass barcode and pass id display do not show shine/skeleton shimmer loading animation', () => {
    ...
    assert.doesNotMatch(indexHtml, /id="epass-barcode"[^>]*skeleton-shimmer/, 'index.html must not have skeleton-shimmer on epass-barcode');
    assert.doesNotMatch(indexHtml, /id="pass-id-display"[^>]*skeleton-shimmer/, 'index.html must not have skeleton-shimmer on pass-id-display');
    assert.doesNotMatch(epassJs, /barcodeSvg\.classList\.add\(['"]skeleton-shimmer['"]\)/, 'epass.js must not add skeleton-shimmer to barcodeSvg');
    assert.doesNotMatch(epassJs, /passIdEl\.innerHTML\s*=.*skeleton-shimmer/, 'epass.js must not add skeleton-shimmer to passIdEl');
    assert.doesNotMatch(epassCss, /#epass-barcode\.skeleton-shimmer/, 'epass.css must not have skeleton-shimmer rule for barcode');
    assert.match(epassJs, /barcodeSvg\.innerHTML\s*=\s*['"]['"]/, 'epass.js must clear barcode innerHTML when showing skeleton');
  });
});
